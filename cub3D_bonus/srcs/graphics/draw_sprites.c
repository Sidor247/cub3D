...
/*   Updated: 2021/10/31 00:08:52 by cwhis            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sprite(t_image *img, t_image *tex,
		float *z_buf, t_vector transform)
{
	t_sdraw			sdraw;
	t_point			point;
	t_point			tex_point;
	...
	{
		if (transform.y > 0 && transform.y < z_buf[point.x])
		{
			...
			{
				if (stencil_buf[point.y][point.x])
					continue ;
...
void	draw_sprites(t_image *img, t_data *data)
{
	...
	while (tmp)
	{
		...
		{	
			...
			draw_sprite(img, tmp->tex, data->z_buf, transform);
		}
		tmp = tmp->prev;
	}
}
